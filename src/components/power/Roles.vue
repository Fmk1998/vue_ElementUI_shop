<template>
  <div>
      ...
      <el-table :data="rolelist" style="width: 100%" border stripe>
                ...
                <el-row :class="[i2 === 0 ? '' : 'bdtop','vcenter']" v-for="(item2,i2) in item1.children"
                  ...
                  <el-col :span="18">
                    <el-tag closable @close="removeRightById(scope.row,item3.id)" type="warning"
                            :class="[i3 === 0 ? '' : 'bdtop']" v-for="(item3,i3) in item2.children" :key="item3.id">
                      {{item3.authName}}
                    </el-tag>
                  </el-col>
                </el-row>
        ...
        <el-table-column label="操作" width="300">
          <template v-slot="scope">
            ...
            <el-button type="warning" icon="el-icon-setting" size="mini" @click="showSetRightDialog(scope.row)">分类权限
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!--修改角色对话框-->
    <el-dialog title="修改角色" :visible.sync="editDialogVisible" width="50%" :before-close="handleClose"
    ...
    methods: {
      ...
      async addRole () {
        this.$refs.addFormRef.validate(async valid => {
          if (!valid) return
          //可以发起添加用户的网络请求
          const { data: res } = await this.$axios.post('api/roles', this.addForm)
      ...
      editRoleInfo () {
        this.$refs.editFormRef.validate(async valid => {
          if (!valid) return
          //可以发起编辑角色的网络请求
          const { data: res } = await this.$axios.put('api/roles/' + this.editForm.roleId, {
      ...
      async removeRoleById (id) {
        ...
        //已确认删除，执行删除操作
        const { data: res } = await this.$axios.delete('api/roles/' + id)
        if (res.meta.status !== 200) {
      ...
      async removeRightById (role, rightId) {
        ...
        const { data: res } = await this.$axios.delete(`api/roles/${role.id}/rights/${rightId}`)
        if (res.meta.status !== 200) {
      ...
      async allotRights () {
        ...
        const { data: res } = await this.$axios.post(`api/roles/${this.roleId}/rights`, { rids: idStr })
        if (res.meta.status!==200) {
