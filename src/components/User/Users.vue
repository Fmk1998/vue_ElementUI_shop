  ...
  export default {
    ...
    methods: {
      ...
      async userStateChanged (userinfo) {
        ...
        const { data: res } = await this.$axios.put(`/api/users/${userinfo.id}/state/${userinfo.mg_state}`)
      ...
      addUser () {
        this.$refs.addFormRef.validate(async valid => {
          if (!valid) return
          //可以发起添加用户的网络请求
          const { data: res } = await this.$axios.post('/api/users', this.addForm)
      ...
      editUserInfo () {
        this.$refs.editFormRef.validate(async valid => {
          if (!valid) return
          //可以发起编辑用户的网络请求
          const { data: res } = await this.$axios.put('/api/users/' + this.editForm.id,
            {
      ...
      async removeUserById (id) {
        ...
        //如果用户确认删除，则返回值为自负床confirm
        //如果用户取消了删除，则返回为字符串cancel
        if (confirmResult !== 'confirm') {
          return this.$message.info('已取消删除')
        }
        //已确认删除，执行删除操作
        const { data: res } = await this.$axios.delete('api/users/' + id)
        if (res.meta.status !== 200) {
      ...
      async saveRoleInfo () {
        ...
        const { data: res } = await this.$axios.put(`api/users/${this.userInfo.id}/role`, { rid: this.selectedRoleId })
        if (res.meta.status !== 200) {
